...
/// `TE_TEST_STUCK_TIMEOUT` overrides it for slow devices.
...
fn check() {
    ...
    error!("TE_TEST_STUCK {name} {elapsed}s");
}

fn notify_secs() -> u64 {
    std::env::var("TE_TEST_STUCK_TIMEOUT")
        .ok()
...
fn abort_run(name: &str, elapsed: u64) -> ! {
    ...
    println!("TE_TEST_RESULT {total} tests, {failed} failed");
