...
The engine builds for tvOS and renders in the Apple TV simulator. The test-game menu
...
- `plat` is vendored at `deps/plat` as 0.10.0, a workspace member like the other
  foundational crates. `ios` means ios or tvos, a separate `tvos` alias marks the real
  differences, `mobile` and `apple` include tvos, and `Platform` has a `TVOS` const.
  Every in-tree consumer points at the path, so one copy resolves everywhere.
...
- The three `target_os = "ios"` gates in `test-engine/Cargo.toml` include tvos.
...
```bash
cargo build -p test-game --lib --target aarch64-apple-tvos     -Z build-std=std,panic_abort  # device
cargo build -p test-game --lib --target aarch64-apple-tvos-sim -Z build-std=std,panic_abort  # simulator
```
...
test-mobile template, which today generates iOS projects only.

- `TestGame/main.m` calls `test_engine_start_app()`, same as iOS.
- `TestGame/test-engine.h` defines the five `test_engine_ios_*` symbols. They are the
  iOS implementations as is, UIAlertController and UITextField exist on tvOS, except
  `test_engine_ios_get_icloud_storage_path` returns NULL, tvOS has no iCloud document
  storage.
- `TestGame/Info.plist` is minimal plus `UIDeviceFamily` = 3, Apple TV.
...
```bash
mkdir -p mobile/tvOS/build/TestGame.app
cp mobile/tvOS/TestGame/Info.plist mobile/tvOS/build/TestGame.app/
cp -R assets mobile/tvOS/build/TestGame.app/assets
xcrun -sdk appletvsimulator clang -target arm64-apple-tvos26.2-simulator -fobjc-arc \
  mobile/tvOS/TestGame/main.m target/aarch64-apple-tvos-sim/debug/libtest_game.a \
  ...
  -o mobile/tvOS/build/TestGame.app/TestGame
codesign -s - --force mobile/tvOS/build/TestGame.app
```
...
xcrun simctl install <udid> mobile/tvOS/build/TestGame.app
