...
use crate::deps::refs::{
    AsAny, Erased, RawPointer, Rglica, ToRglica, ref_counter::RefCounter, weak_from_ref,
};
...
impl<T: ?Sized> Weak<T> {
    ...
    fn check(&self, check_main: bool) {
        ...
        assert!(
            !check_main || crate::deps::hreads::is_main_thread(),
        ...
        if self.is_null() {
            ...
            let message = format!(
                ...
                self.type_name,
                crate::deps::refs::pointers_info::PointerInfo::get_info(self.addr())
            );
