...
use crate::deps::{
    ...
    refs::{AsAny, PTR_SIZE, RawPointer, Weak, ref_counter::RefCounter},
};
...
impl<T: Sized + 'static> Own<T> {
    ...
    pub fn new(val: T) -> Self {
        ...
        // #[cfg(feature = "stats")]
        // crate::deps::refs::stats::adjust_stat(type_name, 1);
...
impl<T: ?Sized> Own<T> {
    ...
    fn check() {
        assert!(
            crate::deps::hreads::is_main_thread(),
            "Unsafe Own pointer deref: {}. Thread is not Main. Thread id: {}",
            std::any::type_name::<T>(),
            crate::deps::hreads::current_thread_id()
        );
    }
}
...
impl<T: ?Sized> Drop for Own<T> {
    ...
    fn drop(&mut self) {
        ...
        // #[cfg(feature = "stats")]
        // crate::deps::refs::stats::adjust_stat(self.type_name, -1);
        #[cfg(feature = "pointers_info")]
...
mod tests {
    ...
    use crate::deps::{hreads::set_current_thread_as_main, refs::Own};
