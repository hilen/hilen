//! The command line test runner. `ui-test` and any app with its own UI
//! tests call `run_cli` from their `main` with the tests they cover, so
//! every runner takes the same flags.

use std::{collections::BTreeMap, env::var, panic::set_hook, path::PathBuf, process::exit, sync::OnceLock};
...
/// Every test this runner covers, handed in by `run_cli`.
static TESTS: OnceLock<fn() -> BTreeMap<String, UITestEntry>> = OnceLock::new();

fn all_tests() -> BTreeMap<String, UITestEntry> {
    TESTS.get().expect("run_cli sets the tests first")()
}
...
fn run(args: Args) -> Result<()> {
    ...
    anyhow::ensure!(
        ...
        "No UI tests registered. Either the `hilen/ui-tests` feature is off, or a linker \
         dropped a test crate whose ctors nothing references, see `run_cli`.",
    );
    ...
    if args.display.present {
        ...
        anyhow::ensure!(
            ...
            "UI test not found: {name}. Run with --list to see every registered test."
        );
    ...
    let actor = async move {
        ...
        if let Some(test_name) = test_name {
            ...
            if missing {
                eprintln!("Run with --list to see every registered test.");
                exit(1);
            }
...
/// Parses the command line and runs the tests `tests` returns. `tests`
/// must name every crate whose tests it covers before it reads the
/// registry, a crate whose ctors nothing references is dropped by the
/// linker with all its tests and nothing reports it.
pub fn run_cli(tests: fn() -> BTreeMap<String, UITestEntry>) -> Result<()> {
    TESTS.get_or_init(|| tests);
    run(Args::parse())
}
