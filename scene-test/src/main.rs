...
struct DisplayArgs {
    ...
    /// Build one test's scene over the whole window and hand it over,
...
fn run(args: Args) -> Result<()> {
    ...
    anyhow::ensure!(
        !tests.is_empty(),
        "No scene tests registered. The `hilen/scene-tests` feature is off.",
    );
    ...
    for name in &names {
        if !tests.contains_key(name) {
            eprintln!("Scene test not found: {name}");
            missing = true;
        }
    }
    if missing {
        eprintln!("Run `cargo run -p scene-test -- --list` to see every registered test.");
        exit(1);
    }
    ...
    if failures.is_empty() {
        println!("{total} scene tests passed");
        return Ok(());
    }
...
fn report_failures(total: usize, failures: &[TestFailure]) {
    ...
    eprintln!("\n{} of {total} scene test(s) failed:", unique.len());
    for f in &unique {
        eprintln!("  - {}", f.name);
    }
